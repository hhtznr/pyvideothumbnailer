...
def create_preview_thumbnails(params: PyVideoThumbnailerParameters, file_path: str) -> None:
    ...
    if not params.no_header:
        ...
        text_height_file_info = get_font_height(file_info, header_font)
        text_height_size_info = get_font_height(size_info, header_font)
        text_height_video_info = get_font_height(video_info, header_font)
        text_height_audio_info = get_font_height(audio_info, header_font)
        text_height_comment = 0
        if comment is not None:
            text_height_comment = get_font_height(comment, header_font)
    ...
    for row_index in range(rows):
        ...
        for column_index in range(columns):
            ...
            timestamp_height = get_font_height(formatted_time, timestamp_font)
...
def get_bool_store_action(b: bool) -> str:
    """
...
def parse_args() -> Namespace:
    ...
    parser.add_argument('--no-header',
                         action=get_bool_store_action(not DEFAULT_NO_HEADER),
    ...
    parser.add_argument('--override-existing',
                         action=get_bool_store_action(not DEFAULT_OVERRIDE_EXISTING),
    ...
    parser.add_argument('--recursive',
                         action=get_bool_store_action(not DEFAULT_RECURSIVE),
    ...
    parser.add_argument('--raise-errors',
                         action=get_bool_store_action(not DEFAULT_RAISE_ERRORS),
    ...
    parser.add_argument('--verbose',
                         action=get_bool_store_action(not DEFAULT_VERBOSE),
...
def get_parameters() -> PyVideoThumbnailerParameters:
    ...
    if os.path.isfile(config_file):
        ...
        if CONFIG_SECTION_LAYOUT in config:
            ...
            if 'no_header' in layout_options:
                params.no_header = config.getboolean(CONFIG_SECTION_LAYOUT, 'no_header')
    ...
    if args.recursive is not DEFAULT_RECURSIVE:
    ...
    if args.no_header is not DEFAULT_NO_HEADER:
        params.no_header = args.no_header
    ...
    if args.override_existing is not DEFAULT_OVERRIDE_EXISTING:
        params.override_existing = args.override_existing
    if args.output_directory is not None:
        params.output_directory = args.output_directory
    if args.raise_errors is not DEFAULT_RAISE_ERRORS:
        params.raise_errors = args.raise_errors
    if args.verbose is not DEFAULT_VERBOSE:
